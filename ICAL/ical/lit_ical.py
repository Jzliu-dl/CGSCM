...
from ical.utils.utils import (
    ExpRateRecorder, Hypothesis, ce_loss, focal_loss_with_ignore,
    to_bi_tgt_out, plicit_tgt_out)

from ical.utils.generation_pred import generate_default_label_set, generate_new_label_set, out_hat_to_pred
...
class LitICAL(pl.LightningModule):
    ...
    def forward(
        ...
        FloatTensor
            [2b, l, vocab_size]
        """
        return self.ical_model(img, img_mask, text, text_mask, tgt)
    ...
    def training_step(self, batch: Batch, _):
        ...
        if self.current_epoch > 20:
            out_hat ,_,_,_ = self(batch.imgs, batch.mask, pred_paddded,pred_mask, exp_tgt)
            pred = out_hat_to_pred(out_hat, 5)
            ogl_loss = ce_loss(out_hat, exp_tgt)
            pred_paddded, pred_mask, error_mask = generate_new_label_set(pred, batch.indices, self.device)

        exp_out_hat, imp_out_hat, fusion_out_hat,msk_hat = self(
            batch.imgs, batch.mask, pred_paddded,pred_mask, exp_tgt)
        ...
        self.log("train_fusion_loss", fusion_loss,
                 on_step=False, on_epoch=True, sync_dist=True)
        self.log("train_msk_loss", msk_loss,
                    on_step=False, on_epoch=True, sync_dist=True)
        self.log("train_ogl_loss", ogl_loss,
                    on_step=False, on_epoch=True, sync_dist=True)

        loss = exp_loss + implicit_loss + fusion_loss+msk_loss+ogl_loss
    ...
    def validation_step(self, batch: Batch, _):
        ...
        pred_paddded, pred_mask, error_mask = generate_default_label_set(len(batch.indices), self.device)
        hyps,_ = self.approximate_joint_search(batch.imgs, batch.mask, pred_paddded, pred_mask)
        pred_paddded, pred_mask, error_mask = generate_new_label_set([h.seq for h in hyps], batch.indices, self.device)


        exp_out_hat, imp_out_hat, fusion_out_hat, msk_hat = self(batch.imgs, batch.mask, pred_paddded, pred_mask, exp_tgt)
    ...
    def approximate_joint_search(
        ...
        return self.ical_model.beam_search(img, mask, text,text_mask,**self.hparams)
