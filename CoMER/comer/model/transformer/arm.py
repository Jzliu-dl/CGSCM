...
class AttentionRefinementModule(nn.Module):
    ...
    def forward(
        self, prev_attn: Tensor, key_padding_mask: Tensor, h: int, w: int,curr_attn: Tensor
    ) -> Tensor:
        """
        Parameters
        ----------
        ...
        Tensor
            [(b * nhead), t, l]
        """
        # prev_attn: [(b * nhead), t, (h * w)+l]
        # key_padding_mask: [b, (h * w)+1]

        img_prev_attn = prev_attn[:, :, : h * w]
        img_curr_attn = curr_attn[:, :, : h * w]
        img_key_padding_mask = key_padding_mask[:, : h * w]
        text_curr_attn = curr_attn[:, :, h * w :]

        curr_attn = img_curr_attn
        prev_attn = img_prev_attn
        ...
        cov = torch.cat([cov, torch.zeros_like(text_curr_attn)], dim=2)
        return cov
