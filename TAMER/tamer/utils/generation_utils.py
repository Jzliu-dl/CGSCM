...
class DecodeModel(pl.LightningModule):
    ...
    def beam_search(
        ...
        src: List[FloatTensor],
        src_mask: List[LongTensor],
        text: List[FloatTensor],
        text_mask: List[LongTensor],
        beam_size: int,
        max_len: int,
        alpha: float,
        early_stopping: bool,
        ...
        rev_scores = self._rate(src, src_mask, text, text_mask, tgt, out, alpha, temperature)
        ...
        _, sim = self.transform(
            [src[0].repeat(2, 1, 1, 1)], [src_mask[0].repeat(2, 1, 1)], [text[0].repeat(2, 1, 1)],[text_mask[0].repeat(2, 1)], tgt
        )
    ...
    def _beam_search(
        ...
        while cur_len < max_len and not beam_scorer.is_done():
            next_token_logits = (
                self.transform(src, src_mask, text, text_mask, input_ids)[0][
                    :, -1, :] / temperature
            )
            # [b *, l, v]
            ...
            if cur_len == 1:
                ...
                for i in range(len(src)):
                    ...
                    text[i] = repeat(text[i], "b ... -> (b m) ...", m=beam_size)
                    text_mask[i] = repeat(text_mask[i], "b ... -> (b m) ...", m=beam_size)
    ...
    def _rate(
        ...
        """
        b = tgt.shape[0]
        out_hat = self.transform(src, src_mask,text,text_mask, tgt)[0] / temperature
